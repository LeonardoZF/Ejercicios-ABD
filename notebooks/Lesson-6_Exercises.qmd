...
Para llevar a cabo la simulación se necesita encontrar el valor de n (tamaño de la muestra) tal que el error en la estimación de la media sea menor que 0.01 con un nivel de confianza del 99\%. Es decir:
$$
P\left( \left| \hat{\theta} - \mu \right| < 0.01 \right) \geq 0.99
$$
Dado que el objetivo es estimar la media de la distribución previa mediante simulaciones, podemos considerar $\hat{\theta}$ como el promedio de n valores simulados de la variable aleatoria $\theta$. Por el Teorema Central del Límite, este promedio se aproxima a una distribución normal con media $\mu$ y error estándar dado por:
$$
\text{SE}_{\text{MC}} = \sqrt{\frac{\text{Var}(\theta)}{n}}
$$
donde $\text{SE}_{\text{MC}}$ representa el error estándar Monte Carlo.

Queremos que este error sea suficientemente pequeño para que la estimación de la media esté dentro de un margen de error $\varepsilon = 0.01$ con un nivel de confianza del 99\%, lo cual se se logra si se cumple la siguiente desigualdad:
$$
z \cdot \text{SE}_{\text{MC}} \leq \varepsilon
$$
donde $z = qnorm(0.995) \approx 2.576$ es el cuantil de la normal estándar correspondiente. 

Sustituyendo la expresión del error estándar, obtenemos:
$$
z \cdot \sqrt{\frac{\text{Var}(\theta)}{n}} \leq \varepsilon
$$
Ahora, para saber el tamaño de la muestra suficiente, necesitamos despejar n:
$$
n \geq \frac{\text{Var}(\theta)}{\left( \frac{\varepsilon}{z} \right)^2}
$$
Finalmente, sustituimos los valores para obtener n:
```{r, echo=FALSE}
...
# Valores teóricos
media_teorica <- a / (a + b) #Esto se utilizará después
var_teorica <- (a * b) / ((a + b)^2 * (a + b + 1))

alpha <- 0.01              # Nivel de confianza
z <- qnorm(1 - alpha / 2)  # Cuantil z para 99% de confianza
epsilon <- 0.01            # Precisión deseada
n_sim <- var_teorica / (epsilon / z)^2
```

$$
n \geq \frac{`r var_teorica`}{\left( \frac{`r epsilon`}{`r z`} \right)^2} \approx `r ceiling(n_sim)`
$$
Ya teniendo el tamaño muestral deseado, se pueden hacer las simulaciones: 
```{r}
library(tidyverse)
set.seed(20250327) #Tal vez ya no es necesario
theta_sim <- rbeta(n_sim, a, b)
theta_tibble <- tibble(theta = theta_sim)

# Resultados
theta_tibble |> 
  summarise( 
    media_sim   = mean(theta),
    varianza_sim = var(theta),
    media_t = media_teorica,
    varianza_t = var_teorica
  )
```
Como último paso se representa la distribución resultante:
```{r}
theta_tibble |> 
  ggplot(aes(x = theta)) +
  geom_density(color = "blue") +
  labs(
    title = "Distribución previa simulada (Beta(1,1))",
    x = expression(theta),
    y = "Densidad"
  )
```
...
```{r}
set.seed(20250327)
sim_pre<- rbinom(n = length(theta_sim), size = n_muestra_1, prob = theta_sim)
predictiva_pre_tibble <- tibble(y=sim_pre)
predictiva_pre_tibble |> 
  ggplot(aes(x = y)) +
  geom_bar() +
  labs(
    title = "Distribución predictiva previa (n = 22)",
    x = "Número de respuestas 'sí' simuladas",
    y = "Frecuencia"
  )
```
La distribución es casi totalmente plana, es decir que refleja la  incertidumbre previa (todos los resultados entre 0 y 22 son plausibles). Esto es compatible con la distribución previa no informativa. 
:::
...
```{r}
obs_1 <- aceptacion_muestra_1 |> 
  filter(resp_descarga_app == "Si") |> 
  nrow() # Convierte el tiible en una sola fila
centil <- mean(sim_pre <= obs_1)
prob_mayor_igual <- mean(sim_pre >= obs_1)
prob_menor_igual <- centil

# Resultados
tibble(
  observado = obs_1,
  centil_empírico = centil * 100,
  P_mayor_igual = prob_mayor_igual,
  P_menor_igual = prob_menor_igual
)
```
:::
...
```{r}
set.seed(20250327)
n_obs <- nrow(aceptacion_muestra_1)

# Parámetros de la distribución posterior por conjugación
a_post <- a + obs_1
b_post <- b + n_obs - obs_1

# Simulación de la distribución posterior
theta_post <- rbeta(n_sim, a_post, b_post)
posterior_tibble <- tibble(theta = theta_post)

posterior_tibble |> 
  ggplot(aes(x = theta)) +
  geom_density(color="blue") +
  labs(
    title = expression("Distribución posterior Beta(18, 6)"),
    x = expression(theta),
    y = "Densidad"
  ) 
...
```{r}
set.seed(20250327)
sim_post <- rbinom(n = length(theta_post), size = n_obs, prob = theta_post)
predictiva_post_tibble <- tibble(y = sim_post)
predictiva_post_tibble %>%
  ggplot(aes(x = y)) +
  geom_bar() +
  labs(
    title = "Distribución predictiva posterior (n = 22)",
    x = "Número de respuestas 'sí' simuladas",
    y = "Frecuencia"
  ) 
```
...
a_post_2 <- a + obs_2
b_post_2 <- b + n_muestra_2 - obs_2

# Simulación de la distribución posterior
theta_post_2 <- rbeta(n_sim, a_post_2, b_post_2)
posterior_2_tibble <- tibble(theta = theta_post_2)
posterior_2_tibble |> 
  ggplot(aes(x = theta)) +
  geom_density(color="blue") +
  labs(
    title = expression("Distribución posterior_2 (Beta (66,49))"),
    x = expression(theta),
    y = "Densidad"
  )
```
...
sim_post_2 <- rbinom(n = length(theta_post_2), size = n_muestra_2, prob = theta_post_2)
predictiva_post_2_tibble <- tibble(y = sim_post_2)

predictiva_post_2_tibble %>%
  ggplot(aes(x = y)) +
  geom_bar() +
  labs(
    title = "Distribución predictiva posterior (n= 113)",
    x = "Número de respuestas 'sí' simuladas",
    y = "Frecuencia"
  )
...
```{r}
set.seed(20250327)
pred_post_2_pequeña <- rbinom(n = length(theta_post_2), size = n_muestra_1, prob = theta_post_2)
pred_post_2_pequeña_tibble <- tibble(y = pred_post_2_pequeña, modelo = "Posterior Muestra 2")

# Etiquetar la PP de muestra 1
predictiva_post_1_tibble <- tibble(y = sim_post, modelo = "Posterior Muestra 1")

# Combinar ambas
predictiva_ambas <- bind_rows(predictiva_post_1_tibble, pred_post_2_pequeña_tibble)

# --- Paso 5: Graficar ambas distribuciones con línea vertical en y_obs = 17 ---
predictiva_ambas |> 
  ggplot(aes(x = y, fill = modelo)) +
  geom_bar() +
  geom_vline(xintercept = obs_1, color = "black", linetype = "dashed") +
  labs(
    title = "Distribuciones predictivas posteriores para n = 22",
    x = "Número de respuestas 'sí' simuladas",
    y = "Frecuencia",
...
```{r}
# Probabilidades bajo el modelo ajustado con segunda muestra
P_mayor_igual_m2 <- mean(pred_post_2_pequeña >= obs_1)
P_menor_igual_m2 <- mean(pred_post_2_pequeña <= obs_1)

# Resultados
tibble(
  Observado = obs_1,
  P_mayor_igual = P_mayor_igual_m2,
  P_menor_igual = P_menor_igual_m2
)
```
En el gráfico anterior se ve que la mayor parte de la distribución (0.979; color cyan) queda a la izquierda del valor empírico de la muestra 1 (17).
:::
...
Aquí lo que se necesita preguntar es ¿qué tan probable es observar un valor como el de la muestra 1 (17 respuestas 'sí') bajo el modelo ajustado con la muestra 2?
No tengo claro cómo calcularía un valor p pero entiendo que sería similar a lo hecho en la pregunta 8, es decir que el valor dado representaría la probabilidad de obtener un resultado tan extremo (o más) que el observado en la muestra 1, bajo el modelo ajustado con la muestra 2. Este valor sería muy pequeño (0.0519), por lo que la hipótesis 'nula' (ambas muestras vienen de la misma población) sería rechazada. 
:::
...
```{r}
set.seed(20250327)
# Simulación predictiva posterior con el modelo ajustado con muestra 1, pero n de muestra 2
pred_post_1_grande <- rbinom(n = length(theta_post), size = n_muestra_2, prob = theta_post)

P_mayor_igual_m1 <- mean(pred_post_1_grande >= obs_2)
P_menor_igual_m1 <- mean(pred_post_1_grande <= obs_2)

# Mostrar resultados
tibble(
  Observado_2 = obs_2,
  P_mayor_igual = P_mayor_igual_m1,
  P_menor_igual = P_menor_igual_m1,
)

```
Aquí sucede lo contrario al caso anterior: solo el 5.08% de las simulaciones del modelo ajustado con la muestra 1 predicen un número de respuestas “sí” menor o igual a 65 en una muestra de tamaño 113. Eso implica que el resultado observado en la muestra 2 es estadísticamente inesperado tomando en cuenta el modelo ajustado con la muestra 1. Por lo tanto, la muestra 2 no es muy compatible con el modelo generado a partir de la muestra 1, lo que apoya la idea de que ambas provienen de poblaciones distintas.
:::
...
posterior_lambda |> 
  ...
  geom_density(alpha = 0.6, color = "black") + #Contorno y transparencia
  labs(
...
# Simulación de la distribución predictiva posterior
y_pred_post_sin <- rpois(n_sim, lambda_sin)
y_pred_post_con <- rpois(n_sim, lambda_con)

pred_post_lambda <- bind_rows(
  tibble(y = y_pred_post_sin, grupo = "Sin título universitario"),
  tibble(y = y_pred_post_con, grupo = "Con título universitario")
)

pred_post_lambda |> 
  ggplot(aes(x = y, fill = grupo)) +
  geom_bar(position = "identity", alpha = 0.6, color = "black") + #Barras superpuestas
  labs(
    title = "Distribuciones predictivas posteriores de nº de hijos",
    x = "Número de hijos",
    y = "Frecuencia",
    fill = "Grupo"
  )
...
```{r}
# Filtrar mujeres con 4 o más hijos
fert_4 <- fertilidad_gss_1990 |> 
  filter(n_hijos >= 4) |> 
  group_by(titulo_uni) |> 
  summarise(total = sum(frecuencia), .groups = "drop") #Desagrupar ya hecha la manipulación

# Probabilidades condicionales
total_4 <- sum(fert_4$total) # P(B)

fert_4 <- fert_4 |> 
  mutate(prob_condicional = total / total_4) # P(A int B) / P(B)

# Calcular odds
odds_sin_vs_con <- fert_4$prob_condicional[fert_4$titulo_uni == "sin"] /
                   fert_4$prob_condicional[fert_4$titulo_uni == "con"]
...
```{r}
mean(y_pred_post_con > y_pred_post_sin)
```
...
No tengo nada claro cómo responder esta pregunta.
:::
...
prop_obs <- fertilidad_gss_1990 |>
  group_by(n_hijos) |>
  summarise(
    total_con = sum(frecuencia[titulo_uni == "con"]),
    total_sin = sum(frecuencia[titulo_uni == "sin"]),
    total = total_con + total_sin,
    prop_con = total_con / total) |>
  mutate(tipo = "Observada")

# Predictiva posterior para con título universitario según el número de hijos
sim_con <- tibble(n_hijos = y_pred_post_con) #Transformar vector en tibble

# Proporción de predictiva posterior
prop_sim <- sim_con |>
  group_by(n_hijos) |>
...
prop_comb <- bind_rows(prop_obs, prop_sim)

prop_comb |>
  ggplot(aes(x = n_hijos, y = prop_con, color = tipo)) +
  geom_line() +
  labs(
    title = "Proporción de mujeres con título universitario según nº de hijos",
    x = "Número de hijos",
    y = "Proporción",
    color = ""
  )
...
```{r}
max_empírico <- fertilidad_gss_1990 |>
  filter(titulo_uni == "con") |>
  summarise(max_n_hijos = max(n_hijos)) |>
  pull()

tibble(max_emp=max_empírico,max_pred=max(y_pred_post_con))
```
...
max_poisson <- function(lambda, n) {
  muestra <- rpois(n, lambda)   # simular muestra de tamaño n de Poisson(lambda)
  max_valor <- max(muestra)     # tomar el máximo de la muestra
  return(max_valor)
}
```
:::
...
```{r}
set.seed(20250327)
maximos_simulados <- sapply(lambda_con, max_poisson, n = n_con)

# Calcular el valor-p predictivo posterior
ppp_max <- mean(maximos_simulados <= max_empírico)
ppp_max
```
En el 95.1% de las simulaciones el valor máximo es igual o menor al empírico, es decir que ese valor máximo no es nada inusual. El modelo ajustado es compatible con lo empírico. 
:::
...
```{r}
media_empirica_con <- fertilidad_gss_1990 |>
  filter(titulo_uni == "con") |>
  summarise(media = weighted.mean(n_hijos, frecuencia)) |> #Media ponderada
  pull()

set.seed(20250327)
medias_simuladas <- sapply(lambda_con, function(x) {
  mean(rpois(n_con, x))
})

ppp_media <- min(1, 2 * min( #Forzar a que máximo sea 1
  mean(medias_simuladas <= media_empirica_con),
  mean(medias_simuladas >= media_empirica_con)))
ppp_media
```
El valor‑p predictivo posterior obtenido fue 1. Esto quiere decir que el valor observado del estadístico (la media) es compatible con el modelo ajustado. Tal vez este estadístico no es muy informativo finalmente. 
:::
